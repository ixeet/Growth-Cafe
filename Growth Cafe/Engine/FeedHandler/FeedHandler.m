    ...
    [manager POST:GET_UPDATE_URL parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (NSDictionary *dicContent in [responseDic objectForKey:@"feedList"]) {
            ...
            for (NSDictionary *dicComment in arrayTempComments) {
                ...
                NSMutableArray * arraySubComments= [[NSMutableArray alloc]init];
                if (subcomment  !=nil) {
                    for (NSDictionary *dicSubComment in subcomment) {
                        Comments *subComment= [[Comments alloc]init];
                        if([dicSubComment objectForKey:@"likeCounts"]!=nil){
                            
                            subComment.likeCounts=[NSString stringWithFormat:@"%@",[dicSubComment objectForKey:@"likeCounts"]];
                        }
                        if([dicSubComment objectForKey:@"shareCounts"]!=nil){
                            
                            
                            subComment.shareCounts=[NSString stringWithFormat:@"%@",[dicSubComment objectForKey:@"shareCounts"]];
                        }
                        if([dicSubComment objectForKey:@"commentCounts"]!=nil){
                            
                            subComment.commentCounts=[NSString stringWithFormat:@"%@",[dicSubComment objectForKey:@"commentCounts"]];
                        }
                        subComment.commentById=[NSString stringWithFormat:@"%@",[dicSubComment objectForKey:@"commentById"]];
                        subComment.commentId=[dicSubComment objectForKey:@"commentId"];
                        
                        subComment.parentCommentId=[dicSubComment objectForKey:@"parentCommentId"];
                        subComment.commentBy=[dicSubComment objectForKey:@"commentBy"];
                        subComment.commentByImage=[dicComment objectForKey:@"commentByImage"];
                        //                    if(comment.commentByImage!=nil){
                        //                        if(comment.commentByImageData==nil)
                        //                        {
                        //
                        ...
                        subComment.commentTxt=[dicSubComment objectForKey:@"commentTxt"];
                        subComment.isLike=[NSString stringWithFormat:@"%@",[dicSubComment objectForKey:@"isLiked"]];
                        subComment.commentDate=[dicSubComment objectForKey:@"commentDate"];
                        
                        [arraySubComments addObject:subComment];
                    }
                }
                comment.subComments=arraySubComments;
